...
def main():
    cap = cv2.VideoCapture("../../AllVideos/testVideos/video1.mp4")
    length = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    print( length )

    print('Deleting files in TrainingData...')
    ...
    with open(r'C:\Users\User\Desktop\Court Athena\Tesseract-OCR\images\video1_classified\groundTruth.json') as jsonFile:
        ...
        while cap.isOpened:
            ...
            # if len(groundTruthJson["Score"][index]["groundTruth"]) != 0 and (groundTruthJson["Score"][index]["groundTruth"][2] >= 10 or groundTruthJson["Score"][index]["groundTruth"][3] >= 10):
            if len(groundTruthJson["Score"][index]["groundTruth"]) != 0:
                ...
                for idx, data in enumerate(cropData):
                    ...
                    if NumberClass >= 10:
                        ...
                        # leftDw = leftDw+4
                        # Dh = Dh+4
                        scoreboard = cv2.rectangle(scoreboard, (Dx, Dy), (Dx+leftDw, Dy+Dh), (255, 0, 0), 1)
